...
const GroupStackedDendrogram = props => {
  ...
  useEffect(() => {
    ...
    if (secondary && tertiary) {
      for (let i = 0; i < graphData.length; i++) {
        tempArray.push(graphData[i][tertiary]);
        setTertiaryList(tempArray);
      }
      tempArray = [];
    }
    if (!tertiary) {
  ...
  useEffect(() => {
    ...
    // // Draw Graph using the data and updated dimensions
  ...
  const treeStructure = d3
    .tree()
    .size([320 - margin.top - margin.bottom, dimensions.width * 0.2]);
