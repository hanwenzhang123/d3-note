...
const GroupedStackedGraph = ({
  ...
  const updateXaxis = (xScale, leftMargin, maxDomain) => {
    const xAxis = axisBottom(xScale).tickFormat(xAxisTickFormat).tickSize(10);
    svg
      .select(".x-axis")
  ...
  const updateGroupedBars = (
    ...
    barg
      ...
      .on("mouseover", function (sequence) {
        ...
        select(this).style("stroke-width", 2).style("stroke", "#333");
      })
  ...
  useEffect(() => {
    ...
    let maxDomain = max(layers, (layer) =>
      max(layer, (sequence) => sequence[1])
    );
  ...
  useEffect(() => {
    g.selectAll(".layer").selectAll("rect").style("stroke-width", 0);
    if (!activeLegend) return;
    const layers = g.selectAll(
      ".sbc" + activeLegend.replace(/[^a-zA-Z0-9_]/g, "_")
    );
    layers.selectAll("rect").style("stroke-width", 2).style("stroke", "#333");
  }, [activeLegend]);
